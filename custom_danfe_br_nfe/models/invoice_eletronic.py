...
# * - Realizadas modificações para utilizar prefixo (Origem/Item) nos itens ao gerar o xml. 
# * - Ex.: OV0001-01 PRODUTO A.
#          OV0001-02 PRODUTO B.
...
class CustomInvoiceEletronic(models.Model):
    ...
    def _prepare_eletronic_invoice_values(self):
        ...
        #Update Module - Start
        res = super(CustomInvoiceEletronic, self)._prepare_eletronic_invoice_values()
        #Update Module - End
 ...
 SEM VALOR FISCAL'
        ...
        for item in self.eletronic_item_ids:
            ...
            eletronic_items.append(
                self._prepare_eletronic_invoice_item(item, self, index))
        #Update Module - End
        total = {
            # ICMS
    ...
    @api.multi
    #Update Module - Start
    def _prepare_eletronic_invoice_item(self, item, invoice, index=0):
    #Update Module - End
        res = super(CustomInvoiceEletronic, self)._prepare_eletronic_invoice_item(
            item, invoice)
        if self.model not in ('55', '65'):
            return res
 ...
 SEM VALOR FISCAL'
        ...
        #Update Module - Start
        index_with_zero = str(index).zfill(2)
        compl = '{0}-{1} '.format(self.invoice_id.origin, index_with_zero)
        #Update Module - End
